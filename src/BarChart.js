	...
	changeBars(secondary) {
		...
		data.forEach((row, i) => {
			...
			if (!acc[secondary_val]) {
				...
				acc[secondary_val] = {
					avg:
						(prev.avg * prev.count + parseFloat(primary_val)) /
						(prev.count + 1),
					count: prev.count + 1,
				};
			}
		...
		// Bars
		const bar_iter = svg.selectAll("mybar").data(acc).join("g");
		bar_iter
			.append("rect")
			.attr("x", (d) => x(d[0]))
			.attr("y", (d) => y(d[1].avg))
					...
					{cat_columns.map((col, index) => (
						<span key={col}>
							<input
								...
								onChange={(e) => this.changeBars(e.target.value)}
							></input>
							{col}
							<br></br>
						</span>
